...
#set y interval
	...
	plt.plot(x_data, y_datas[ARRAY], 'r-o', lw = line_size, ms = dot_size)
	plt.xlabel(f"n\nfrom 2^{MIN_POWER_N} to 2^{MAX_POWER_N}", fontsize = font_size)
	plt.ylabel("time(sec)", fontsize = font_size)
	plt.title(f"compare hash table vs array on inserting datas", fontsize = font_size)
	...
	plt.legend(["hash table", "array"], fontsize = font_size)
