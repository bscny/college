...
int main(int argc,char **argv){
	vector<int> array_inssort;
    vector<int> array_msort;
    vector<int> array_lumoto;
    vector<int> array_hoare;
    vector<int> array_3way;
    vector<int> array_countsort;
	...
	for(int ii = 10; ii <= MAX_N; ii++){
		n = pow(2, ii);
		array_inssort.resize(n);
		array_msort.resize(n);
		array_lumoto.resize(n);
		array_hoare.resize(n);
		array_3way.resize(n);
		array_countsort.resize(n);
