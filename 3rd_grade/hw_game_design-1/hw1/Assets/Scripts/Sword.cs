...
public class Sword : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (GameObject.Find("StickerMan").GetComponent<StickerMan>().Get_isOnGround() == true)
        {
            this.transform.rotation = Quaternion.Euler(0, 0, 0);
        }
        else
        {
            d_x = GameObject.Find("StickerMan").GetComponent<StickerMan>().Get_dir_X();
            if (d_x > 0)
            {
                d_x = 1;
            }
            ...
            if (GameObject.Find("StickerMan").GetComponent<StickerMan>().Get_Velocity() > 0) {
                this.transform.rotation = Quaternion.Euler(0, 0, d_x * 45);
            }
            else if (GameObject.Find("StickerMan").GetComponent<StickerMan>().Get_Velocity() < 0)
            {
                this.transform.rotation = Quaternion.Euler(0, 0, d_x * -45);
            }
            else if (GameObject.Find("StickerMan").GetComponent<StickerMan>().Get_Velocity() == 0)
