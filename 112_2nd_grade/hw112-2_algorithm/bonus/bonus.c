...
void find_all_node_in_the_route(int head_name, tNode *cur_vertice, int accumulated_val, char *mother_routes, char *current_DFS_check, int check_times,
	...
	}else if(strcmp(current_DFS_check, mother_routes) == 0){
		accumulated_val += cur_vertice->edge_length;
	}else{
	...
	while(temp != NULL){
		if(strcmp(temp->by_route, current_DFS_check) == 0){
			find_all_node_in_the_route(head_name, temp, accumulated_val, current_DFS_check, current_DFS_check, check_times + 1,
...
int survay(tNode **adj_list, int max_name, int target_from, int target_to){
	...
	for(int i = 1; i <= max_name; i ++){
		if(i == target_from){
			...
			strcpy(vehicle_to_node[i], "0");

			bottom_up_heap(heap, addr_nodes_in_heap, top);
		}else{
			//  init invisible nodes
			heap[top].vertice_name = i;
			heap[top].key = INF;
	...
	while(top > 0){   // each ans_distance
		...
		addr_nodes_in_heap[current_node] = &heap[top];
		heap[0] = heap[top];
		heap[top] = temp;
		...
		while(each_adj_node != NULL){
			find_all_node_in_the_route(current_node, each_adj_node, ans_distance[current_node], vehicle_to_node[current_node], each_adj_node->by_route, 0,
									   ans_distance, addr_nodes_in_heap, vehicle_to_node, heap, adj_list);
